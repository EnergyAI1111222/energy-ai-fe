...
import { Newspaper, Send, TrendingUp, AlertCircle, MessageSquare, ExternalLink } from "lucide-react";

export default function InsightsPage() {
  const reports = [
    { category: "MARKET VOLATILITY", time: "2m ago", title: "TTF Spot prices surge 4% following outage at Kollsnes processing plant.", impact: "HIGH", color: "text-red-500", content: "Supply constraints in the North Sea are driving short-term bullish sentiment." },
    { category: "RENEWABLES", time: "45m ago", title: "German wind output expected to hit record 48GW this weekend.", impact: "NEUTRAL", color: "text-blue-500", content: "Negative pricing likely in the Day-Ahead auction for Sunday delivery." },
    { category: "REGULATORY", time: "3h ago", title: "ACER releases new guidance on cross-border capacity allocation.", impact: "LOW", color: "text-emerald-500", content: "Minimal immediate impact on NTC values, but long-term structural changes expected." }
  ];
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
              ...
              <button className="w-full py-3 bg-slate-50 text-slate-900 rounded-lg text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-slate-100 transition-all border border-slate-100">
                 DOWNLOAD FULL REPORT <ExternalLink className="w-3 h-3" />
              </button>
           </div>
        </div>
      </div>
...
function InsightCard({ category, time, title, impact, color, content }: any) {
  return (
